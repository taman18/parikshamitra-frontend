...
//   );
  ...
  export const getQuestions = createAsyncThunk(
    "getQuestions",
    async ({ accessToken ,limit, page}: { accessToken: string ,page:number, limit:number}) => {
      const getQuestionsApiResponse = await fetch(
        `${process.env.NEXT_PUBLIC_DEV_BASE_URL}/admin/question/get-questions?page=${page}&&limit=${limit}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    extraReducers: (builder) => {
      builder
        ...
        .addCase(getQuestions.fulfilled, (state, action) => {
          state.loading = false;
          const transformedData: QuestionInterface[] =
